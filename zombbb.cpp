...
void Game::run() {
	...
	uint32_t quiv, i, kld = 0, m = 1, rnd = 0, iter;
	...
	while (!(graveyard.size() >= mZomb.size() && cin.fail())) {
		...
		if (round == rnd && !cin.fail()) {
			...
			for (iter = 0; iter < nzomb; ++iter) {
				...
				que.push(&mZomb.back());
				if (verb) {
					cout << "Created: " << mZomb.back().name << " (distance: " << mZomb.back().dist
						<< ", speed: " << mZomb.back().spd << ", health: " << mZomb.back().hlth << ")\n";
				}
			}
			cin >> disp >> nzomb;
			for (iter = 0; iter < nzomb; ++iter) {
